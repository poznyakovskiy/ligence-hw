...
@app.post("/image")
async def image_post(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    for modified_id in range(0, NUM_MODIFIED_IMAGES):
        ...
        # Save the modified image asynchronously
        loop = asyncio.get_event_loop()
        loop.run_in_executor(
            None,
            modified_img.save,
            os.path.join(modified_folder, modified_filename)
        )
